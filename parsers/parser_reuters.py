...
def parse_reuters_news():
    ...
    for article in articles:
        try:
            ...
            client.execute(
                'INSERT INTO news.reuters_headlines (title, link, content, rubric, source, category) VALUES',
                [(title, link, content, rubric, 'reuters.com', category)]
            )
            
            # Сохранение в категорийную таблицу
            category_table = f'news.reuters_{category}'
            try:
                client.execute(
                    f'INSERT INTO {category_table} (title, link, content, source, category, published_date) VALUES',
                    [(title, link, content, 'reuters.com', category, datetime.now())]
                )
            except Exception as e:
